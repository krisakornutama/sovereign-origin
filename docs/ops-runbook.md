...
| CI: Core API tests | `core-api-tests.yml` — **2 jobs**: `test` (เมทริกซ์ ubuntu+windows × Node 24, mock suite, `TZ=Asia/Bangkok` ตรง prod) + `test-db` (ubuntu + Postgres 15 service container, `prisma db push` แล้วรัน real-DB lifecycle ผ่าน `RUN_DB_TESTS=1` ด้วย harness กลาง `tests/db-harness.ts` — knowledge upload + documents scan-to-inventory + treasury slip + OTA firmware + whisper disk lifecycle: `tests/knowledge-db.test.ts`, `tests/uploads-db.test.ts`) — เขียวครบตั้งแต่ `2027836` |
