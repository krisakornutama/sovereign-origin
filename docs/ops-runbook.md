...
| Required checks บน main | **พร้อมตั้งครบ 6** — `audit`, `size`, `publish` โหมด PR, `Core API tests / test (ubuntu-latest)`, `Core API tests / test (windows-latest)`, **`Core API tests / test-db`** (context ล่าสุดเพิ่มใน `7b90e5c` · ยืนยันจาก check-runs จริงของ commit `1be5fa6`: audit/size/test (ubuntu)/test (windows)/test-db = เขียวครบ 5 ตัว) — **ยัง apply ไม่ได้บนแพลนปัจจุบัน**: private repo บน free โดน HTTP 403 ทั้ง rulesets และ branch protection (ทดสอบซ้ำด้วย `gh api` เมื่อ 17 ก.ย. 2026 แล้ว) พออัปเกรด Pro หรือเปลี่ยน repo เป็น public → รันคำสั่งเดียวจบ — **คู่มือทีละคลิก (พร้อมยืนยันผล + ปัญหาที่เจอ) อยู่ที่ `docs/enable-required-checks.md`** |
