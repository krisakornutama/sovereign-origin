...
เว็บโชว์ผลงานแบบ static หลายหน้า (`index.html` = ทัวร์ทั้งระบบ + `projects.html` = ผลงานทีละชิ้น + ฟอร์มติดต่อ + `livestock.html` = ปศุสัตว์ + `fishery.html` = แบบแปลนการประมง + `farm.html` = เกษตร&สมุนไพร + `iot.html` = บ้าน IoT + `education.html` = โมดูล AI สอนลูก + `books.html` = คลังหนังสือของนักเขียน: ผลงานที่ลงพิมพ์+ลิงก์แหล่งเดิม และชั้นหนังสือที่อ่าน) — ไม่มี build step ไม่มี dependency โฮสต์ฟรีด้วย GitHub Pages
...
2. กด **uploading an existing file** แล้วลากไฟล์จากโฟลเดอร์ `portfolio/` **ทั้ง 10 ไฟล์** ขึ้นไป (`index.html`, `projects.html`, `livestock.html`, `fishery.html`, `farm.html`, `iot.html`, `education.html`, `books.html`, `.nojekyll`, `README.md`)
...
git init
git add index.html projects.html livestock.html fishery.html farm.html iot.html education.html books.html .nojekyll README.md
